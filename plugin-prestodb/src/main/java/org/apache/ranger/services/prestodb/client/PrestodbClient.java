...
public class PrestodbClient extends BaseClient implements Closeable {
  public static final String PRESTODB_USER_NAME_PROP = "user";
  public static final String PRESTODB_PASSWORD_PROP = "password";
  ...
  private void initConnection() {
    ...
    Properties prestodbProperties = new Properties();
    prestodbProperties.put(PRESTODB_USER_NAME_PROP, prop.getProperty(HadoopConfigHolder.RANGER_LOGIN_USER_NAME_PROP));
    if (prop.getProperty(HadoopConfigHolder.RANGER_LOGIN_PASSWORD) != null) {
      prestodbProperties.put(PRESTODB_PASSWORD_PROP, prop.getProperty(HadoopConfigHolder.RANGER_LOGIN_PASSWORD));
    }
    ...
    if (driverClassName != null) {
      try {
        ...
        String msgDesc = "initConnection: unable to initiate connection to PrestoDB instance,"
        ...
        String msgDesc = "initConnection: Unable to connect to PrestoDB instance, "
    ...
    try {
      con = DriverManager.getConnection(url, prestodbProperties);
    } catch (SQLException e) {
      String msgDesc = "Unable to connect to PrestoDB instance.";
      HadoopException hdpException = new HadoopException(msgDesc, e);
      hdpException.generateResponseDataMap(false, getMessage(e),
        msgDesc + ERR_MSG, null, null);
      throw hdpException;
    ...
    if (con != null) {
      ...
      try {
        if (needle != null && !needle.isEmpty() && !needle.equals("*")) {
          // Cannot use a prepared statement for this as prestodb does not support that
  ...
  private void close(Connection con) {
    try {
      ...
      LOG.error("Unable to close PrestoDB SQL connection", e);
    }
  }
