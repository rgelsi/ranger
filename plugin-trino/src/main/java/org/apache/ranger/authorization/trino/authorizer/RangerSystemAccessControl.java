...
public class RangerSystemAccessControl
  ...
  public void checkCanSetSystemSessionProperty(SystemSecurityContext context, String propertyName) {
    if (!hasPermission(createSystemPropertyResource(propertyName), context, TrinoAccessType.ADMIN)) {
  ...
  public void checkCanCreateRole(SystemSecurityContext context, String role, Optional<TrinoPrincipal> grantor) {
    if (!hasPermission(createRoleResource(role), context, TrinoAccessType.ADMIN)) {
      LOG.debug("RangerSystemAccessControl.checkCanCreateRole(" + role + ") denied");
  ...
  public void checkCanDropRole(SystemSecurityContext context, String role) {
    if (!hasPermission(createRoleResource(role), context, TrinoAccessType.ADMIN)) {
      LOG.debug("RangerSystemAccessControl.checkCanDropRole(" + role + ") denied");
  ...
  public void checkCanGrantRoles(SystemSecurityContext context, Set<String> roles, Set<TrinoPrincipal> grantees, boolean adminOption, Optional<TrinoPrincipal> grantor) {
    for (String role : roles) {
      if (!hasPermission(createRoleResource(role), context, TrinoAccessType.ADMIN)) {
        LOG.debug("RangerSystemAccessControl.checkCanGrantRoles(" + role + ") denied");
  ...
  public void checkCanRevokeRoles(SystemSecurityContext context, Set<String> roles, Set<TrinoPrincipal> grantees, boolean adminOption, Optional<TrinoPrincipal> grantor) {
    for (String role : roles) {
      if (!hasPermission(createRoleResource(role), context, TrinoAccessType.ADMIN)) {
        LOG.debug("RangerSystemAccessControl.checkCanRevokeRoles(" + role + ") denied");
  ...
  public void checkCanShowRoleAuthorizationDescriptors(SystemSecurityContext context) {
    if (!hasPermission(createUserResource(context.getIdentity().getUser()), context, TrinoAccessType.ADMIN)) {
      LOG.debug("RangerSystemAccessControl.checkCanShowRoleAuthorizationDescriptors() denied");
  ...
  public void checkCanShowCurrentRoles(SystemSecurityContext context) {
    if (!hasPermission(createUserResource(context.getIdentity().getUser()), context, TrinoAccessType.ADMIN)) {
      LOG.debug("RangerSystemAccessControl.checkCanShowCurrentRoles() denied");
  ...
  public void checkCanShowRoleGrants(SystemSecurityContext context) {
    if (!hasPermission(createUserResource(context.getIdentity().getUser()), context, TrinoAccessType.ADMIN)) {
      LOG.debug("RangerSystemAccessControl.checkCanShowRoleGrants() denied");
  ...
  public void checkCanInsertIntoTable(SystemSecurityContext context, CatalogSchemaTableName table) {
    if (!hasPermission(createResource(table), context, TrinoAccessType.INSERT)) {
  ...
  public void checkCanDenyTablePrivilege(SystemSecurityContext context, Privilege privilege, CatalogSchemaTableName table, TrinoPrincipal grantee) {
    if (!hasPermission(createResource(table), context, TrinoAccessType.REVOKE)) {
      LOG.debug("RangerSystemAccessControl.checkCanDenyTablePrivilege(" + table + ") denied");
  ...
  public void checkCanSetViewAuthorization(SystemSecurityContext context, CatalogSchemaTableName view, TrinoPrincipal principal) {
    if (!hasPermission(createResource(view), context, TrinoAccessType.GRANT)) {
      LOG.debug("RangerSystemAccessControl.checkCanSetViewAuthorization(" + view.toString() + ") denied");
      AccessDeniedException.denySetViewAuthorization(view.toString(), principal);
    }
  }

  /**
   * Create materialized view is verified on schema level
   */
  @Override
  public void checkCanCreateMaterializedView(SystemSecurityContext context, CatalogSchemaTableName materializedView, Map<String, Object> properties) {
    if (!hasPermission(createResource(materializedView.getCatalogName(), materializedView.getSchemaTableName().getSchemaName()), context, TrinoAccessType.CREATE)) {
      LOG.debug("RangerSystemAccessControl.checkCanCreateMaterializedView(" + materializedView.toString() + ") denied");
  ...
  public void checkCanRefreshMaterializedView(SystemSecurityContext context, CatalogSchemaTableName materializedView) {
    if (!hasPermission(createResource(materializedView), context, TrinoAccessType.UPDATE)) {
      LOG.debug("RangerSystemAccessControl.checkCanRefreshMaterializedView(" + materializedView.toString() + ") denied");
  ...
  public void checkCanSetMaterializedViewProperties(SystemSecurityContext context, CatalogSchemaTableName materializedView, Map<String, Optional<Object>> properties) {
    if (!hasPermission(createResource(materializedView), context, TrinoAccessType.ALTER)) {
      LOG.debug("RangerSystemAccessControl.checkCanSetMaterializedViewProperties(" + materializedView.toString() + ") denied");
  ...
  public void checkCanDropMaterializedView(SystemSecurityContext context, CatalogSchemaTableName materializedView) {
    if (!hasPermission(createResource(materializedView), context, TrinoAccessType.DROP)) {
      LOG.debug("RangerSystemAccessControl.checkCanDropMaterializedView(" + materializedView.toString() + ") denied");
  ...
  default void checkCanUpdateTableColumns(SystemSecurityContext context, CatalogSchemaTableName table, Set<String> updatedColumnNames) {
    for (RangerTrinoResource res : createResource(table, updatedColumnNames)) {
      if (!hasPermission(res, context, TrinoAccessType.UPDATE)) {
        ...
        AccessDeniedException.denySelectColumns(table.getSchemaTableName().getTableName(), updatedColumnNames);
      }
    }
  }
  ...
  public void checkCanViewQueryOwnedBy(SystemSecurityContext context, Identity queryOwner) {
    if (!hasPermission(createIdentityResource(queryOwner), context, TrinoAccessType.IMPERSONATE)) {
  ...
  public void checkCanExecuteFunction(SystemSecurityContext context, String functionName) {
    if (!hasPermission(createFunctionResource(functionName), context, TrinoAccessType.EXECUTE)) {
      LOG.debug("RangerSystemAccessControl.checkCanExecuteFunction(" + functionName + ") denied");
  ...
  public void checkCanExecuteTableProcedure(SystemSecurityContext context, CatalogSchemaTableName table, String procedure) {
    if (!hasPermission(createResource(table), context, TrinoAccessType.EXECUTE)) {
      LOG.debug("RangerSystemAccessControl.checkCanExecuteTableProcedure(" + table.toString() + "," + procedure + ") denied");
  ...
  private static RangerTrinoResource createRoleResource(String role) {
    RangerTrinoResource res = new RangerTrinoResource();
    res.setValue(RangerTrinoResource.KEY_ROLE, role);

    return res;
  }

  private static RangerTrinoResource createIdentityResource(Identity identity) {
    RangerTrinoResource res = new RangerTrinoResource();
    res.setValue(RangerTrinoResource.KEY_USER, identity.getUser());
...
class RangerTrinoResource
  ...
  public static final String KEY_ROLE = "trinorole";
...
enum TrinoAccessType {
  CREATE, DROP, SELECT, INSERT, UPDATE, DELETE, USE, ALTER, ALL, GRANT, REVOKE, SHOW, IMPERSONATE, EXECUTE, ADMIN;
}